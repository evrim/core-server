...
(defmethod layout.root ((self layout))
  (aif (handler-bind ((error #'(lambda (c) (declare (ignore c)) (invoke-restart 'bootstrap:use-nil))))
	 (bootstrap:home)) 
       (pathname it)
       (s-v 'root)))
    ...
    (let ((systems (find-file :pattern "*.asd")))
      (mapcar #'(lambda (sys)
		  (unless (search "_darcs" sys)
		    (with-current-directory (layout.systems self)
		      (ln :source (merge-pathnames (pathname sys) (make-pathname :directory '(:relative :up)))
...
(defmethod install ((self layout)) 
  ...
  (link-systems self)
  (with-current-directory (layout.root self)
    (ln :source (merge-pathnames (merge-pathnames (s-v 'etc) #P"core-server/") (s-v 'lib))
	:target (layout.root self))
    (ln :source (merge-pathnames (merge-pathnames (s-v 'doc) #P"core-server/") (s-v 'lib))
	:target (layout.root self)))
  (write-templates self)
  (chmod :mode "+x" :path (layout.core-server.sh self))
  (shell :cmd +cp+ :args (list "lib.conf" (layout.etc self)))
  (with-current-directory (layout.root self)
    (ln :source (merge-pathnames (merge-pathnames  #P"src/" #P"core-server/") (s-v 'lib))
	:target (layout.root self))
